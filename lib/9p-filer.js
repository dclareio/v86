...
function Virtio9p(filesystem, cpu, bus) {
    ...
    this.replybuffer = new Uint8Array(this.msizeVirtIO)
    // , VirtIO*2); // Twice the msize to stay on the safe side
...
Virtio9p.prototype.set_state = function(state) {
  console.log('********', state)
    ...
    try {
      ...
      this.fids = state[8].reduce(function(acc, [inode, type, uid, path], fid)
      {
          return Object.assign(acc, { [fid]: { type, uid, path , inode} });
      }, {});
    }
